"""
***************************************************************************
Class File:             Runconfig Apache Configuration Module
Authors/Maintainers:    Rich Nason (rnason@appcontainers.io)
Copyright:              Copyright 2016 Richard Nason
Description:            This class will handle all things associated with
                        setting up and configuring the apache web service.
***************************************************************************
"""
# *******************************************************************
# Required Modules:
...
class Apache():
    ...
    def apache_config(self):
        """Backup and Modify the Apache main config file"""
        INSTALL_LOG.write_log_console("Configuring Apache server config file...", "")

        # Ensure that the APP_NAME variable is set to something
        if self.app_name == "" or self.app_name is None:
                self.app_name = gethostname()

        # Backup the original File
        INSTALL_LOG.write_log("Backing up the Apache server config file")
        ...
        if GLOBALS.is_rhel():
            os.popen("sed -i 's/#ServerName\ www.example.com:80/ServerName\ www.$APP_NAME:80/g' /etc/httpd/conf/httpd.conf")
        else:
            try:
